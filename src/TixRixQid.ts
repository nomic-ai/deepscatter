import type { Bool, StructRowProxy, Vector } from 'apache-arrow';
 ...
 * @param deeptable The deepscatter dataset
 * @returns The tile, if it exists.
 *
 */
export async function tixToTile(tix: Tix, deeptable: Deeptable) {
  if (tix === 0) {
    return deeptable.root_tile;
  }
  if (isNaN(tix)) {
  ...
  const parent = await tixToTile(parentTix(tix), deeptable);
  //
  await parent.populateManifest();
  // Now that the parents are loaded, we can find the child.
  const [z, x, y] = tixToZxy(tix);
  const key = `${z}/${x}/${y}`;
  const t = deeptable.map((d) => d).filter((d) => d.key === key);
  if (t.length) {
    return t[0];
  }
...
export function getTileFromRow(row: StructRowProxy, dataset: Deeptable): Tile {
  ...
  const matches = dataset
    ...
    .filter((t) => {
      ...
      const relatedRow = t.record_batch.get(rix);
      if (relatedRow === null) {
        return false;
      }
      return relatedRow.ix === ix;
    });

  if (matches.length === 0) {
    throw new Error('No tiles found for this row.');
  }
  return matches[0];
}
